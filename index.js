...
app.post('/send-text', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, error: 'Failed to send message for unknown reason.' });
  } catch (error) {
...
app.post('/send-image-url', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ success: false, error: 'Failed to send image for unknown reason.' });
  } catch (error) {
